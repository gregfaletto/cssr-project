...
* genLatentData generates jointly Gaussian data sets that include clusters of features that are correlated with a latent feature (the latent features themselves are also provided).
...
* genLatentData
  - checkGenLatentDataInputs checks that the inputs to genLatentData are as expected.
...
Tests for checkGenLatentDataInputs
...
Finally, tests for genLatentData
...
#' of X_predict are named, they must match the names of X_train_selec.
#' @param clusters Optional; either an integer vector of a list of integer
#' vectors; each vector should contain the indices of a cluster of features (a
#' subset of 1:p). (If there is only one cluster, clusters can either be a list
#' of length 1 or an integer vector.) All of the provided clusters must be
...
#' @author Gregory Faletto, Jacob Bien
#' @export
cssPredict <- function(X_train_selec, y_train_selec, X_predict, clusters=list(),
    ...
    css_results <- css(X_train_selec, y_train_selec, lambda, clusters,
        train_inds=train_inds)
