...
The cssr package implements [cluster stability selection (Faletto and Bien 2022)](https://arxiv.org/abs/2201.00494), a feature selection method designed to allow stability selection to work effectively in the presence of highly correlated features.

> 📖 **Want to see how the package works under the hood?** `cssr` is built with [literate programming](https://jacobbien.github.io/litr-project/) — the full implementation and its explanation live together in the [bookdown that defines the package](https://gregfaletto.github.io/cssr-project/create/).
...
Given data $(X, y)$, cluster stability selection selects the variables in $X$ that are useful for predicting $y$.
...
Cluster stability selection is an extension of [stability selection (Meinshausen and Bühlmann, 2010)](https://arxiv.org/abs/0809.2932). Stability selection is a procedure designed to make any feature selection procedure more *stable*. Given a data set $(\boldsymbol{X}, \boldsymbol{y})$ with $n$ observations, stability selection works as follows:
...
![Stability Selection Flowchart Figure](man/figures/stability-selection-diagram.png "Stability Selection")
...
![Selection Proportions](man/figures/sel_props.png "Proxy Selection Proportions")
...
* **Prediction** --- `vignette("prediction", "cssr")` walks through predicting `y` for new observations using cluster representatives (`cssPredict()`, `css()` + `getCssPreds()`, `getCssDesign()`, and the weighting schemes).
* **Advanced usage** --- `vignette("advanced-usage", "cssr")` covers the competitor methods (`protolasso()`, `clusterRepLasso()`), the data generators used in the paper's simulations, and other helpers.
* The [bookdown that defines the package](https://gregfaletto.github.io/cssr-project/create/) contains the fully annotated source.
