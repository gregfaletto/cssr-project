...
testthat::test_that("getSelectedClusters returns the whole tied set when raising the threshold would empty it (#203)", {
  # The max loop can leave more than one way, and this block pins the way
  # nothing else detects. Exit A, `if(thresh > B) break`, fires when the tied
  # clusters sit TOGETHER at count B -- selection proportion exactly 1.0 --
  # which is what "plot.cssr surfaces the tie-breach warning (#159c)" reaches.
  # The loop can also end on its own `while` condition, having succeeded: that
  # is what "getSelectedClusters max_num_clusts handles proportion-1.0 ties
  # (#42)" does, because its c1 at count B and c2 at count B - 1 are NOT tied,
  # so c2 drops out and the condition goes false before either break runs.
...
testthat::test_that("getSelectedClusters overshoots min_num_clusts on a whole-count tie, silently (#203)", {
  ...
  # assumed. #206 asks whether that silence is intended -- checkSelectedClusters()
  # warns on the min_num_clusts UNDERshoot and says nothing on the overshoot.
  # This assertion is what makes resolving #206 the other way a visible change
  # rather than a silent one, which is the point of pinning it while the
  # question is open.
