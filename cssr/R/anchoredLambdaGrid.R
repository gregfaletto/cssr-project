...
anchoredLambdaGrid <- function(lambda_path, s, n_interior=5L){
    ...
    # elements that lands here. Pinned end to end by
    # test_that("cssLasso survives a degenerate lasso path (#125)"), which drives
    # such a design through cssLasso() and asserts an empty selection rather than
    # an error. Note that the repo's older #157 fixture does NOT reach this
    # function: it goes through clusterLassoCore(), which never calls cssLasso().
