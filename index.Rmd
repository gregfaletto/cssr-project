...
Subsamples \(A_1, \ldots, A_B \subset [n]\) of size \(\lfloor n/2 \rfloor\) are drawn, as well as subsamples \( \overline{A}_b \subset [n] \) of the same size with \(A_b \cap \overline{A}_b = \emptyset\).
...
The main inputs that `css()` accepts are...
...
* and a specification of the type of subsamples.  For now we only consider the complementary subsets sampling of Shah and Samworth.  However, we have included an option for Meinshausen-Buhlmann sampling which we may add later:
...
# Tests for main functions in ``r params$package_name`` (now that helper functions have been defined)
