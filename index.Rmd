...
usethis::create_package(
  ...
  fields = list(
    ...
    Description = "Implementation of Cluster Stability Selection, a feature selection
      method proposed in Faletto and Bien (2022) <arXiv:2201.00494>.",
    Language = "en-US",
...
Notice that `cssLasso()` depends on the package `glmnet`, and calls the function `checkCssLassoInputs()`, which verifies that the inputs to `cssLasso()` are as needed. (In particular, the function `css()` imposes very few requirements on `lambda` and `y` to allow the end user flexibility for any specified `fitfun`. `checkCssLassoInputs()` ensures that `y` is a real-valued response, `lambda` is a nonnegative real number, etc.) `cssLasso()` also calls `anchoredLambdaGrid()`, which builds the short penalty grid the second lasso fit runs over--the grid is chosen so that the coefficients at the target penalty come out bit-identical to the full-path refit, at the price of one extra fit over a much shorter grid: it keeps the path down to the target penalty, a little geometric padding below it, and the path's own final penalty, which the interpolation reads and which therefore cannot be dropped. Both `checkCssLassoInputs()` and `anchoredLambdaGrid()` are specified in the helper functions section later.
...
* `anchoredLambdaGrid()` builds the penalty grid that `cssLasso()` fits its second lasso path over, so that the solution at the target penalty is obtained without solving most of the path below it. Most, not all: the grid also retains a few padding penalties and the path's final one, because the interpolation that reads the solution depends on both endpoints of the grid it is given.
...
`anchoredLambdaGrid()` builds the penalty grid that `cssLasso()`'s second `glmnet` fit runs over, replacing the full-path refit that `predict.glmnet(exact = TRUE)` used to perform internally (#125):
...
`snapLambdaToGrid()` recovers the penalty `glmnet()` actually fitted from the penalty `cssLasso()` asked for, so that `predict.glmnet()` reads a solved lasso path column instead of blending two adjacent ones (#199):
...
fixed in #13), the common checks live in three shared helpers:
`checkGenClusteredDataInputsPre()` (the checks that precede each generator's
correlation-specific checks), `checkGenClusteredDataInputsPost()` (the checks
that follow them), and `checkGenClusteredDataInputsSnrSigma()` (the checks on
whichever of snr and sigma_eps_sq was supplied). The two weighted validators
additionally share their rho_high / rho_low checks, in `checkRhoHighLow()`.
Each validator calls `Pre()`, then its own correlation/strength checks, then
`Post()`, then `SnrSigma()` -- preserving each validator's exact original
check order.
...
#' (computationally expensive) subsampling procedure every time it is called.
...
#' `y[i]` is the response corresponding to observation `X[i, ]`. Unlike the more
...
versions, so `cssLasso()` must not care which one it gets. **For the scalar `s` used
here**, glmnet 4.x returns a `data.frame` on a non-empty selection and a list on an
empty one; glmnet 5.0 always returns a list. That is a consequence of the general rule,
not the rule itself -- 4.x coerces to a `data.frame` whenever `apply()` could simplify,
which for one `s` value means whenever anything was selected. The `clusterLassoCore()`
blocks further below cover the no-`s` case, where the same rule produces two *different*
`data.frame` layouts (#190). These blocks mock `glmnet::predict.glmnet` to hand `cssLasso()` each
shape directly, which pins the contract on any machine regardless of which glmnet is
installed -- the point being that a test exercising only the *installed* version pins
nothing about the other one.

The first block is the regression test for #188 proper: it fails on the pre-fix source at
`stopifnot(is.data.frame(pred))`. The other two are pins rather than red-green tests --
they pass on the pre-fix source too, and exist to hold the empty-selection and 4.x
contracts against this refactor.
...
The blocks that follow cover the anchored-grid rewrite of `cssLasso()` (#125), which
replaced `predict.glmnet(exact = TRUE)`'s internal full-path refit with a second
`glmnet()` fit over the short grid `anchoredLambdaGrid()` builds. They divide the
work deliberately, and the division is the thing to understand before changing any of
them: the first block **pins** the output against the reference the rewrite replaced
and passes on the pre-change source too; the second is the **red-green regression
test**, and pins routing rather than construction; the third is what actually
**enforces the grid construction**; and the fourth covers the on-grid short-circuit,
which is a correctness requirement rather than a speed one. The degenerate-path block
and the `snapLambdaToGrid()` blocks further down carry their own narratives.
...
The regression test proper. It captures how `cssLasso()` calls `predict.glmnet()`
and asserts that the call is no longer an exact refit and that the object handed
over is the anchored fit:
...
The invariants of `anchoredLambdaGrid()` itself, which is the block that enforces the
grid construction. It calls the helper directly rather than mocking it: mocking a
`cssr`-internal binding does not work in the bare weave, and there is nothing to gain
from it here anyway.
...
Finally the on-grid short-circuit, which exists to keep the exported direct-call
route byte-identical rather than to save a fit:
...
`anchoredLambdaGrid()`'s guard branch has a reason that is easy to mistake for a hypothetical:
a perfectly orthogonal design makes `glmnet()` return a `$lambda` of `NaN` followed by zeros,
and the guard is what keeps `log(0)` out of the interior-point construction on such a path. The
next block pins that end to end, and in particular pins the decision **not** to validate
`$lambda` at runtime: a positivity check on it would turn this case from an empty selection into
an error, on exactly the shape issue #157 was filed to stop crashing.
...
The three blocks below cover the penalty snap (#199) -- `cssLasso()` predicting at
the entry `glmnet` reported rather than at the penalty it asked for. They divide the
work as deliberately as the #125 blocks do: the first is the **red-green regression
test**, and is the one criterion 5 names -- though the third reddens under the same
mutation too, so a reader scoring it should expect two assertions to move rather
than one; the second **pins the helper's arithmetic**
against hand-built vectors, and owns the tolerance; the third drives the real
`cssLasso()` end to end and asserts that what comes back is a **solved lasso column's**
nonzero set rather than a blend of two.
...
The helper's own arithmetic, pinned directly against hand-built vectors:
...
And the end-to-end effect: the coefficient vector `cssLasso()` reads back is a solved
lasso path column, so its non-selected entries are exact zeros rather than a blend's
residue.
...
These two functions are the package's only `cv.glmnet` call sites, and #186's fix
is a `grouped` argument on each. This block is what observes that fix:
...
no `col`, the method's own highlight colors still apply. The second checks that
...
fs::file_copy("../source-files/WORDLIST", "inst/WORDLIST", overwrite = TRUE)
fs::file_copy("../source-files/NEWS.md", "NEWS.md", overwrite = TRUE)
```
