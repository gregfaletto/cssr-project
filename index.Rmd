...
versions, so `cssLasso()` must not care which one it gets. glmnet 4.x returns a
`data.frame` on a non-empty selection and a list on an empty one; glmnet 5.0 always
returns a list. These blocks mock `glmnet::predict.glmnet` to hand `cssLasso()` each
