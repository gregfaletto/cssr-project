...
At build time we generate the package's reference documentation, README, and pkgdown website: `litr::document()` writes the `man/` pages and `NAMESPACE`; the README is rendered from `source-files/README.Rmd` (its two figures copied into `man/figures/`); the `CITATION` file is installed so `citation("cssr")` and the pkgdown "Citation" page cite the paper (arXiv:2201.00494), and the `WORDLIST` file is installed alongside it so that `devtools::spell_check()` accepts this package's domain vocabulary (proper nouns, function and column names, and identifier fragments) and flags only genuine misspellings; the **Exploring results**, **Prediction**, and **Advanced usage** vignettes are added from `source-files/` via `litr::add_vignettes()`; and `litr::add_pkgdown()` builds the site (including the vignettes under Articles).
