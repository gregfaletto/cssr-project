...
penalties, and 61 on the `clusterRepLasso()` design block B uses). Each mock
therefore sizes its fixture from `ncol(object$beta)` rather than hard-coding a slot count,
which would be a `stopifnot()` error rather than a legible test failure.

Only the single-column `data.frame` block is a red-green regression test for the container
bug proper. The multi-column and list blocks are pins that pass on the pre-fix source too;
the last three pin the boundary's entry-count assertion and the consumer's two container
guards, each of which is otherwise observed by nothing.

The boundary's own `is.list()` / `!is.data.frame()` pair and the inner `nrow()` check in the
row-major branch are deliberately *unpinned*: all three are unreachable once the reshaping has
run, and stand as defence-in-depth against a glmnet returning a container this code has not
been taught. Deleting any of them leaves every block below green, which is the expected result
