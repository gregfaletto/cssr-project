...
`fitfunFailureMessage()` builds the informative error that `getSelMatrix()` raises when a subsample's `fitfun` result is not a valid integer vector (reached only under parallel forking, where the failure arrives as a captured `try-error`):
