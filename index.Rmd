...
Several of the entry points below accept either a numeric matrix or a data.frame for the design matrix and share the same coercion logic (convert the data.frame with `model.matrix`, drop the intercept column, and guard against a column-count change that would invalidate supplied cluster indices). We factor that logic into a single internal helper, `coerceDataFrameToMatrix()`, so the six call sites stay in sync.
