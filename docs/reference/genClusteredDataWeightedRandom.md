...
(the responses from y without the added noise).
...
- rho_high:
  ...
  cluster with the latent variable. Must be a single number in (0, 1\]
  (greater than 0 and at most 1), and at least as large as rho_low.
  Default is 1.
...
- rho_low:
  ...
  cluster with the latent variable. Must be a single number in (0, 1\]
  (greater than 0 and at most 1), and no larger than rho_high. Default
  is 0.5.
...
- Z:
  ...
  The latent features, as an n by n_clusters numeric matrix (one column
  per latent variable). Note that (X, Z) is multivariate Gaussian.
