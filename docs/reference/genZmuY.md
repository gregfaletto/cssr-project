...
- other_X:
  ...
  A numeric matrix of n observations from a multivariate normal
  distribution generated using the specified parameters, containing the
  weak signal features and the noise features that will eventually be
  in X. (The only missing features are the proxies for the latent
  features Z.)
