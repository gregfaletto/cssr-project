...
- clusters:
  ...
  themselves). Default is list() (so no clusters are specified).
...
- x:
  ...
  The provided X, converted to a matrix if it was provided as a
  data.frame, and with column names removed.
...
- prototypes:
  ...
  An integer vector whose length is equal to the number of clusters.
  Entry i is the index of the feature belonging to cluster i that is
  most highly correlated with y (that is, the prototype for the cluster,
  as in the protolasso; see Reid and Tibshirani 2016).

- var_names:

  If the provided X matrix had column names, the names of the featurrs
  in the provided X matrix. If no names were provided, feat_names will
  be NA.
