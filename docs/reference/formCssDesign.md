...
Create design matrix of cluster representatives from matrix of raw
features using results of css function
...
- cutoff:
  ...
  is 0 (in which case all clusters are returned in decreasing order of
...
- newx:
  ...
  must be provided to formCssDesign.) Default is NA.

## Value

A design matrix with the same number of rows as newx (or the train_inds
provided to css) where the columns are the constructed cluster
representatives.
