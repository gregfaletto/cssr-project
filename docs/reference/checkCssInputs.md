# Helper function to confirm that inputs to the function `css()` are as expected, and modify inputs if needed

Helper function to confirm that inputs to the function [`css()`](css.md)
are as expected, and modify inputs if needed
...
- fitfun:
  ...
  forward stepwise selection or LARS and lambda could be the desired
  model size; or fitfun could be the elastic net and lambda could be a
  length-two vector specifying lambda and alpha. Default is cssLasso, an
  implementation of lasso (relying on the R package glmnet), where
  lambda must be a positive numeric specifying the L1 penalty for the
  lasso.
...
- B:
  ...
  sampling_type="SS" the number of subsamples will be `2*B`. Default is
  100 for sampling_type="MB" and 50 for sampling_type="SS".
...
- prop_feats_remove:
  ...
  prop_feats_remove\*p features will be dropped (though this number will
  vary). This is similar in spirit (but distinct from) extended
  ...
  sampling_type="SS", the features dropped in each complementary pair of
  subsamples are identical in order to ensure that the theoretical
...
- train_inds:
  ...
  function getCssPreds after feature selection. (This will only work if
  y is real-valued, because getCssPreds using ordinary least squares
...
- feat_names:
  ...
  A character vector containing the column names of X (if the provided X
  had column names). If the provided X did not have column names,
  feat_names will be NA.

- X:

  The provided X, converted to a matrix if it was originally provided as
  a data.frame, and with feature names removed if they had been
  provided.

- clusters:

  A list of integer vectors; each vector will contain the indices of a
  cluster of features. Any duplicated clusters provided in the input
  will be removed.
