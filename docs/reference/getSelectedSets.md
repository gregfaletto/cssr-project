...
Converts a selected set from X_glmnet to selected sets and selected
clusters from the original feature space of X.
...
- lasso_set:
  ...
  A vector containing the indices of selected cluster representatives or
  prototypes.
...
- prototypes:
  ...
  prototype for the cluster, as in the protolasso).
...
- selected_set:
  ...
  An integer vector with length equal to lasso_set containing a set of
  selected features in the original X matrix. (Selections in lasso_set
  corresponding to a cluster will be replaced by the cluster's prototype
  from X.)

- selected_clusts_list:

  A named list of integer vectors with length equal to selected_set.
  `selected_clusts_list[[k]]` will be an integer vector containing the
  indices of the features in X that are in the cluster containing
  prototype `selected_set[k]`.
