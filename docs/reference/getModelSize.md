...
dropped from X except the one feature with the highest marginal
correlation with y, as in the protolasso (Reid and Tibshirani 2016).
...
- clusters:
  ...
  features that are in a common cluster, as in the output of css. (The
...
An integer; the estimated size of the model. The minimum returned value
is 1, even if the lasso with cross-validation chose no features.
