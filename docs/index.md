...
> 📖 **Want to see how the package works under the hood?** `cssr` is
> built with [literate
> programming](https://jacobbien.github.io/litr-project/) — the full
> implementation and its explanation live together in the [bookdown that
> defines the
> package](https://gregfaletto.github.io/cssr-project/create/).
...
- The data are divided into $`B`$ subsamples of size $`n/2`$ (typically,
  $`B`$ might be around 50 or 100).
...
- Finally, we use the selection proportions to get a selected set (for
  example, by taking the top $`s`$ features for some predetermined
  $`s`$).
...
Figure](reference/figures/stability-selection-diagram.png "Stability Selection")

Stability Selection Flowchart Figure
...
important for predicting $`y`$. $`Z`$ is not observed, but 10 *proxies*
for $`Z`$ are observed–features that are highly correlated with $`Z`$
(and therefore with each other, too). There are also 10 “weak signal”
...
the $`Z`$ proxies would be more useful for predicting $`y`$ than all of
the “weak signal” features.
...
![Selection
Proportions](reference/figures/sel_props.png "Proxy Selection Proportions")
...
cluster of 10 features (specifically, the first 10 columns of $`X`$)
that are highly correlated both with each other and also with an
unobserved variable $`Z`$ that is associated with $`y`$. The cssSelect
function in the cssr package yields a set of features selected by
cluster stability selection. We can tell cssSelect about the cluster
using the “clusters” argument.
...
$`\lambda`$. cssSelect will choose $`\lambda`$ automatically by default,
but you can also provide your own $`\lambda`$ value if you’d like.
(Remember that $`\lambda`$ should be chosen for subsamples of size
$`n/2`$.)
...
- You can select your own number of subsamples to use $`B`$. Larger
  values of $`B`$ may be a bit more robust, but will require more
  computation time. (Also, there will be diminishing returns past a
  certain point, since there are only $`n`$ data points to work with no
  matter how large $`B`$ gets.) The default $`B`$ is 50, leading to 100
  subsamples.
